const article = {
  ...
  author: 'Shannon',
  detail:
`先看一个真实的失败案例。你问检索器"这笔订单的退款政策是什么",embedding 检索器从库里捞回 top-3:一条讲退货流程的,一条讲运费险的,一条讲优惠券的,没一条直接回答。答案其实就在库里,只是它的每个词都和问题对不上——embedding 把整段压成一颗向量,精确词被稀释成了泛语义。同样的候选集换给 reranker,它把问题和每段文档逐字对看一遍,答案跳到了第一名。

前一篇《RAG from Scratch》说过,检索质量决定 RAG 的上限。这个上限由两个模型决定:embedding(双塔 bi-encoder)在入库时把全库文档压成一颗向量,reranker(单塔 cross-encoder)在查询时让查询和文档逐 token 互看再打分。它们回答同一个问题——"这段文档跟你的问题相不相关"——判断方式却南辕北辙:一个在入库时就把判断做完了,一个把判断留到查询时。

这篇文章不讲怎么搭 RAG 流水线(那在前一篇),深入流水线的两个引擎内部:它们为什么长成"双塔"和"单塔"、怎么被训出来、为什么这两年越变越大。最后会看到,这两个模型不是对手——它们是同一份判断力的老师和学生。
...
- **EOS-last 的 recency bias**:取最后一个 token,表示容易被靠近结尾的词主导(因果注意力下越靠后的词看到的越多)。工程上还有一坑:批内句子长度不一、要 left-padding,"取最后一个"必须按 attention mask 找真实位置,不能直接取 [:, -1]。
...
怎么撑?靠损失函数。现在最主流的是 InfoNCE(van den Oord 2018,arXiv:1807.03748),思想简言之:让每个 query 的正文档,在 batch 里排第一。具体说,一个 batch 里除了 query 自己和它该匹配的正文档,其余都是这个 batch 里其他样本(自动当负样本);把 query 和所有文档的相似度算出来,过 softmax,目标是正文档那一项分数最大。这本质是个分类问题:把"对的"从一堆里挑出来。
...
这里要说明现实的限制:上面的规模是几十亿美元和几千张卡堆出来的。E5-Mistral 那约 1.8 亿 token 的合成数据,按当时的 API 价格粗算,光生成就要上百万美元量级,而且合成完还要一致性去重,实际有效率远低于生成量。对个人来说,务实的路径是——**约 1 万对标注 + 单卡,几天就能微调出一个够用的模型**,不需要从头训(想更省就用 LoRA 这类只调少量参数的方式)。判断"够用"的标准:拿你自己的语料,用现成模型生成正负对,微调一个小模型,量一下 Recall@5 有没有涨——这比花几周攒数据快得多。
...
维度不是越多越好,是"够用 vs 成本"。Matryoshka Representation Learning(MRL,2022,arXiv:2205.13147)的思路:训练时对不同长度的前缀各算一份损失(如 64/128/256/512/1024 维),逼判别信息挤进前几维。于是同一颗向量,你既可以只用前 128 维(省存储),也可以全用(更准)。
...
最后,几件容易踩的坑一并列出(前面讲过的只点一句):① 各向异性——句向量挤成一团、余弦分不开(训练节讲过)② 批内负样本会误伤、大 batch 代价高(训练节讲过)③ 维度不可解释(前一篇说过"每一维都不可解释,意义活在相对位置里")④ bge-reranker-v2-m3 跨语言弱 ⑤ 单向量召回上限:512 维约 50 万篇、1024 维约 400 万篇文档起开始漏召回(Weller 等,arXiv:2508.21038,前一篇提过)。
...
回到开头的失败场景:答案在库里,却没被捞出来——问题出在两个裁判身上。裁判可以换、可以训、可以蒸馏。先把上限量出来(约 50 条真实 query),再把精力用在关键处。LLM 一本正经地编,正是这两个裁判,让"别让它记,让它查"成立。`,
