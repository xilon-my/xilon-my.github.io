...
**问题:一步更新可能走太远。** 策略梯度每次用一批数据只更新一步(第 9 章 REINFORCE 每回合更新一次)。如果一步推得太远,策略性能会掉。为什么会被推得太远?答案藏在目标函数怎么用旧数据上:目标要对新策略取期望,数据却是旧策略采的,中间会冒出一个"比值";这个比值不受约束时,能让一步更新迈得太大。下面先讲比值怎么来,再讲比值为什么危险,最后看 TRPO 和 PPO 分别怎么约束它。
...
$\theta=\theta_{\mathrm{old}}$ 时,每个比值都是 1,目标退化成旧数据上的普通平均;$\theta$ 变了,比值给每个样本重新加权,估计仍然是对的——前提是比值别离 1 太远:比值太大,少数样本的权重被放大,估计方差跟着涨。这正是后面 clip 要管的事。有了它,同一批数据可以多轮更新,不用每轮重新采样。

**为什么比值会让更新走太远。** 用 $r_t$ 乘上优势 $\hat{A}_t$ 当目标(不裁剪的版本):
