...
$\pi(s)\leftarrow\arg\max_a q(s,a)$——Bellman 文章的"决策靠 $Q$"。但不能只根据 $v$ 得到 $q$:$q_{\pi_0}(s,a)=R(s,a)+\gamma\sum_{s'}P(s'|s,a)\,v_{\pi_0}(s')$,其中还需要 $P$ 和 $R$。没有模型时无法直接计算。假设有模型,以 $s_1$ 为例逐动作计算(下一步用 $v_{\pi_0}$ 查表):
