...
RL 的核心循环简言之：看状态 → 按策略选动作 → 拿奖励 → 更新策略。策略就是"看到什么就做什么"的映射，它输出的不是固定动作，而是每个动作的概率分布——既偏向已知好的，又保留探索空间。
...
**动作分块（Action Chunking）**——一次预测一整段未来动作（论文里多数任务预测 16 步），而不是一步。单步预测的问题是相邻动作可能来自不同"模式"，造成抖动；预测整段能保证时序一致，而且对"停在原地"的空闲动作更鲁棒。代价是预测太长反应会变慢，16 步是平衡点。
